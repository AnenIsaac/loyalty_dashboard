...
type SortField = 'customerName' | 'interaction_type' | 'numericAmount' | 'points_awarded' | 'numericDate' | 'optional_note'
...
export function CustomerInteractionsTable({ interactions, customers }: CustomerInteractionsTableProps) {
  ...
  const getCustomerName = (interaction: CustomerInteraction): string => {
    if (interaction.customer_id) {
      ...
      // Walk-in customer - get name from interaction record
  ...
  const processedInteractions = useMemo((): ProcessedInteraction[] => {
    const processed = interactions.map(interaction => ({
      ...interaction,
      customerName: getCustomerName(interaction),
      ...
      processed.sort((a, b) => {
        ...
        switch (sortField) {
          ...
          case 'interaction_type':
            aValue = a.interaction_type
            bValue = b.interaction_type
            break
          case 'numericAmount':
          ...
          case 'points_awarded':
            ...
            break
          case 'numericDate':
            aValue = a.numericDate
            bValue = b.numericDate
            break
          case 'optional_note':
            aValue = a.optional_note || ''
            bValue = b.optional_note || ''
            break
          default:
            return 0
        }
  ...
  const sortableColumns = [
    ...
    { field: 'optional_note' as SortField, label: 'Notes' }
  ]
